...
#define V_REF   1100
#define ADC_0    (ADC1_CHANNEL_4)     /*!< ADC1 channel 4 is GPIO32 */
#define ADC_1    (ADC1_CHANNEL_5)     /*!< ADC1 channel 5 is GPIO33 */
#define ADC_2    (ADC1_CHANNEL_6)     /*!< ADC1 channel 6 is GPIO34 */
#define ADC_3    (ADC1_CHANNEL_7)     /*!< ADC1 channel 7 is GPIO35 */
//#define V_REF_TO_GPIO  //Remove comment on define to route v_ref to GPIO
uint32_t voltage_ADC_0;
uint32_t voltage_ADC_1;
uint32_t voltage_ADC_2;
uint32_t voltage_ADC_3;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        break;
    case ESP_GATTS_READ_EVT: {
        ESP_LOGI(GATTS_TAG, "GATT_READ_EVT_JK2, conn_id %d, trans_id %d, handle %d\n", param->read.conn_id, param->read.trans_id, param->read.handle);
        esp_gatt_rsp_t rsp;
        memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 10;
        voltage_ADC_0 = adc1_get_raw(ADC_0);
        voltage_ADC_1 = adc1_get_raw(ADC_1);
        voltage_ADC_2 = adc1_get_raw(ADC_2);
        voltage_ADC_3 = adc1_get_raw(ADC_3);
        rsp.attr_value.value[0] = voltage_ADC_0>>8;
        rsp.attr_value.value[1] = voltage_ADC_0 & 0xff;
        rsp.attr_value.value[2] = voltage_ADC_1>>8;
        rsp.attr_value.value[3] = voltage_ADC_1 & 0xff;
        rsp.attr_value.value[4] = voltage_ADC_2>>8;
        rsp.attr_value.value[5] = voltage_ADC_2 & 0xff;
        rsp.attr_value.value[6] = voltage_ADC_3>>8;
        rsp.attr_value.value[7] = voltage_ADC_3 & 0xff;
        esp_log_buffer_hex(GATTS_TAG, rsp.attr_value.value, 10);
        // rsp.attr_value.value !!!!!! From ESP To computer;
        ...
        if (esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id, ESP_GATT_OK, &rsp) == ESP_OK){
            ESP_LOGI(GATTS_TAG, "Sent!")            
        }
        else{
            ESP_LOGE(GATTS_TAG, "Send response error\n");
        }
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            ///write.value !!! from computer to ESP
            switch(param->write.value[0]){
                case 0x0d : // Digital Set
                    gpio_set_direction(param->write.value[1], GPIO_MODE_OUTPUT);
                    gpio_set_level(param->write.value[1], param->write.value[2]);
                    ESP_LOGI(GATTS_TAG,"Digital Set Executed");
                break;
                case 0x0e : // Digital Set
                    ledc_set_duty(ledc_ch[param->write.value[1]].mode, ledc_ch[param->write.value[1]].channel, param->write.value[2]);
                    ledc_update_duty(ledc_ch[param->write.value[1]].mode, ledc_ch[param->write.value[1]].channel);
                    ESP_LOGI(GATTS_TAG,"PWM written");
                break;
                default : // Digital Set
                    //gpio_set_level(param->write.value[1], param->write.value[2]);
                    ESP_LOGI(GATTS_TAG,"Default truc %d", param->write.value[0]);
                break;
            }
...
void app_main()
{
    ...
    adc1_config_channel_atten(ADC_0, ADC_ATTEN_0db);
    adc1_config_channel_atten(ADC_1, ADC_ATTEN_0db);
    adc1_config_channel_atten(ADC_2, ADC_ATTEN_0db);
    adc1_config_channel_atten(ADC_3, ADC_ATTEN_0db);
